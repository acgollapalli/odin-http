...
_IO :: struct {
	...
	wsa_fns:         map[win.SOCKET]_WSA_VTable,
}

_WSA_VTable :: struct {
	accept_ex:  win.LPFN_ACCEPTEX,
	connect_ex: win.LPFN_CONNECTEX,
	recvmsg:    win.LPFN_WSARECVMSG,
	sendmsg:    win.LPFN_WSASENDMSG,
}
...
Op_RecvMsg :: struct {
	callback: On_RecvMsg,
	socket:   win.SOCKET,
	header:   win.WSAMSG, // windows equivalent of posix msghdr
	received: int,
	pending:  bool,
}
...
prepare_socket :: proc(io: ^IO, socket: net.Any_Socket) -> net.Network_Error {
	net.set_option(socket, .Reuse_Address, true) or_return
        ...
        win_socket := win.SOCKET(net.any_socket_to_socket(socket))
        io.wsa_fns[win_socket] = init_socket_fns(win_socket);
...
handle_completion :: proc(io: ^IO, completion: ^Completion) {
	switch &op in completion.op {
        ...
        case Op_RecvMsg:
        received, err := recvmsg_callback(io, completion, &op)
        if wsa_err_incomplete(err) {
            io.io_pending += 1
            return
        }
        op.received += int(op.received)
        
        nerr := net.UDP_Send_Error(err)
        if nerr != nil {
            op.callback(completion.user_data,int(op.header.namelen), op.received, nerr)
        } else {
            op.callback(completion.user_data, 0, 0, nil)
        }
        wsa_bufs : [^]win.WSABUF = op.header.lpBuffers
        delete_slice(wsa_bufs[:op.header.dwBufferCount], io.allocator)
        
            
        case Op_SendMsg:
        sent, err := sendmsg_callback(io, completion, &op)
        if wsa_err_incomplete(err) {
            io.io_pending += 1
            return                
        }
...
accept_callback :: proc(io: ^IO, comp: ^Completion, op: ^Op_Accept) -> (source: net.Endpoint, err: win.c_int) {
	ok: win.BOOL
		...
		op.pending = true
            ...
            wsa_fns, fns_ok := io.wsa_fns[op.socket]
            assert(fns_ok, "Socket was not initialized!")
            accept_ex := wsa_fns.accept_ex
...
recvmsg_callback :: proc(io: ^IO, comp: ^Completion, op: ^Op_RecvMsg) -> (received: win.DWORD, err: win.c_int) {
	ok: win.BOOL
        if op.pending {
		flags: win.DWORD
            ok = win.WSAGetOverlappedResult(op.socket, &comp.over, &received, win.FALSE, &flags)
	} else {
		wsa_fns, fns_ok := io.wsa_fns[op.socket]
            assert(fns_ok, "Socket was not initialized!")
            recvmsg:= wsa_fns.recvmsg
            
            err_code := recvmsg^(op.socket, &op.header, &received, win.LPWSAOVERLAPPED(&comp.over), nil)
            ok = err_code != win.SOCKET_ERROR
            op.pending = true
	}
...
sendmsg_callback :: proc(io: ^IO, comp: ^Completion, op: ^Op_SendMsg) -> (received: win.DWORD, err: win.c_int) {
	ok: win.BOOL
        if op.pending {
		flags: win.DWORD
            ok = win.WSAGetOverlappedResult(op.socket, &comp.over, &received, win.FALSE, &flags)
		...
		flags: win.DWORD
            wsa_fns, fns_ok := io.wsa_fns[op.socket]
            assert(fns_ok, "Socket was not initialized!")
            sendmsg:= wsa_fns.sendmsg
            
            err_code := sendmsg^(op.socket, &op.header, flags, &received, win.LPWSAOVERLAPPED(&comp.over), nil)
            ok = err_code != win.SOCKET_ERROR
            op.pending = true
	}
...
net_err_to_code :: proc(err: net.Network_Error) -> os.Platform_Error {
	switch e in err {
        case net.Create_Socket_Error:  return os.Platform_Error(e)
            case net.Socket_Option_Error:  return os.Platform_Error(e)
            case net.General_Error:        return os.Platform_Error(e)
            case net.Platform_Error:       return os.Platform_Error(e)
            case net.Dial_Error:           return os.Platform_Error(e)
            case net.Listen_Error:         return os.Platform_Error(e)
            case net.Accept_Error:         return os.Platform_Error(e)
            case net.Bind_Error:           return os.Platform_Error(e)
            case net.TCP_Send_Error:       return os.Platform_Error(e)
            case net.UDP_Send_Error:       return os.Platform_Error(e)
            case net.TCP_Recv_Error:       return os.Platform_Error(e)
            case net.UDP_Recv_Error:       return os.Platform_Error(e)
            case net.Shutdown_Error:       return os.Platform_Error(e)
            case net.Set_Blocking_Error:   return os.Platform_Error(e)
            case net.Parse_Endpoint_Error: return os.Platform_Error(e)
            case net.Resolve_Error:        return os.Platform_Error(e)
            case net.DNS_Error:            return os.Platform_Error(e)
            case:                          return nil
	}
}
...
init_socket_fns :: proc(subject: win.SOCKET) -> (wsa_fns: _WSA_VTable) {
	load_socket_fn(subject, win.WSAID_ACCEPTEX, &wsa_fns.accept_ex)
        load_socket_fn(subject, win.WSAID_CONNECTEX, &wsa_fns.connect_ex)
        load_socket_fn(subject, win.WSAID_WSARECVMSG, &wsa_fns.recvmsg)
        load_socket_fn(subject, win.WSAID_WSASENDMSG, &wsa_fns.sendmsg)
        return
}
